...
for l, (i, j) in enumerate(tqdm(path[:-1])):
    race_map[i, j] = 'o'

    cheat_points = list(set([(i + dx, j + dy, abs(dx) + abs(dy)) for dx in range(-20, 21) for dy in range(-20, 21) if 2 <= abs(dx) + abs(dy) <= 20]))

    for x, y, cheat_len in cheat_points:
        if 0 <= x < rows and 0 <= y < cols and race_map[x, y] in '.E':
            ...
            saved = len(path) - (new_path_len + cheat_len + l)
